...
const DateTimePicker = React.forwardRef(
  (
    ...
    // Function to check if time is in the future for today's date
    const isFutureTime = (time) => {
      if (!disableFuture || !value) return false;
    ...
    const handleSelect = (newDay) => {
      if (!newDay) return;
      if (disableFuture && isFutureDate(newDay)) return;
      ...
      if (disableFuture && isFutureDate(newDateFull)) return;
    ...
    const handleTimeChange = (newDate) => {
      ...
      if (disableFuture && isFutureTime(newDate)) return;
      onChange?.(newDate);
    };
    ...
    return (
      <Popover>
        ...
        <PopoverContent side="top" align="end" className="flex flex-col space-y-2 p-1 -mb-10 w-full overflow-auto h-full">
          <div className="rounded-md border w-full">
            <Calendar
              ...
              locale={locale}
              className="w-full"
              disabled={(date) => (disableFuture ? date > new Date() : false)}
              {...props}
            />
          </div>
